...
use crate::virtual_memory::{
    LogicalAddress, PAGE_FREE, PageDirectory, PhysicalAddress, assume_direct_mapping, isPageAligned};
...
struct FrameAllocatorInner {
    ...
    kernelDirectory: Option<Box<PageDirectory>>,
...
pub unsafe fn initFrameAllocator(mut kernelDirectory: Box<PageDirectory>, start: PhysicalAddress, end: PhysicalAddress) -> *const PageDirectory {
    ...
    let mut addr = start;
    while addr < end {
        let entry = unsafe { kernelDirectory.tryGetPageEntryMut(LogicalAddress(start)).unwrap() };
        *entry = *entry | PAGE_FREE;
        addr = addr + PAGE_SIZE;
    }

    let ptr = core::ptr::from_ref(&*kernelDirectory);
...
pub fn freeFrame(frame: PhysicalAddress) {
    assert!(isPageAligned(unsafe { assume_direct_mapping::<u8>(frame) }));

    let guard = &mut *allocator.0.lock();
    let Some(kernelDirectory) = guard.kernelDirectory.as_mut() else { return; };

    if guard.regionStart <= frame && frame < guard.regionEnd {
        let Some(entry) = (unsafe { kernelDirectory.tryGetPageEntryMut(LogicalAddress(frame)) })
            else { return; };
...
/// actually allocating.
pub fn reserveFrames(count: usize) -> Result<(), ()> {
    let mut guard = &mut *allocator.0.lock();
...
pub fn unreserveFrames(count: usize) {
    let mut guard = &mut *allocator.0.lock();
...
pub fn fulfillReservedFrame() -> Option<PhysicalAddress> {
    let guard = &mut *allocator.0.lock();
    let kernelDirectory = guard.kernelDirectory.as_mut()?;

    let mut curr = guard.currFrame + PAGE_SIZE;

    while curr != guard.currFrame {
        let entry = unsafe { kernelDirectory.tryGetPageEntryMut(LogicalAddress(curr)) };
